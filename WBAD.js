...
if (!$response) {
  // 新版视频流广告素材 CDN：由 WBAD.lpx 的独立开关控制此请求脚本是否启用。
  if (/^https:\/\/ad\.us\.sinaimg\.cn\//i.test(url)) {
    $done({
      response: {
        status: 204,
...
if (url.includes("/interface/sdk/sdkad.php")) {
  ...
  if (url.includes("/2/cardlist")) {
  ...
  } else if (url.includes("/2/search_hot_word") || url.includes("/2/search/hot_word")) {
    // 搜索框热词、组件热词和聚合热词。必须先于通用 /2/search/ 分支处理。
    removeSearchHotWords(obj);
    ...
    // 投票窗口
    removeVoteInfo(obj);
  } else if (url.includes("/2/client/abserver_list") || url.includes("/2/client/get_grayfeature")) {
    // 仅移除名称明确包含广告含义的 A/B 实验，不影响普通功能实验。
    obj = removeAdExperiments(obj);
  } else if (url.includes("/2/video/flow_stream")) {
    // 新版视频流及其缓存接口。抓包中的广告以 ad_state=1、extend_info.ad 标记。
    cleanNewVideoFlow(obj);
...
function isAd(data) {
  ...
  if (data?.is_ad === 1 || data?.is_ad === true) {
    return true;
  }
  if (data?.ad_object || data?.insert_ad_feed) {
    return true;
  }
  return false;
}

// 新版视频流广告判断。ad_marked 会出现在普通视频中，不能单独作为广告依据。
function isNewVideoFlowAd(data) {
  if (!data || typeof data !== "object") return false;
  if (isAd(data)) return true;
  if (data.ad_state === 1 || data.ad_state === "1") return true;
  if (data?.extend_info?.ad) return true;
  return false;
}

// 兼容 flow_stream 与 flow_stream_cache 的嵌套结构。
function cleanNewVideoFlow(node) {
  if (!node || typeof node !== "object") return;
  if (Array.isArray(node)) {
    for (const item of node) cleanNewVideoFlow(item);
    return;
  }
  for (const key of Object.keys(node)) {
    const value = node[key];
    if (Array.isArray(value) && ["items", "statuses", "lists"].includes(key)) {
      node[key] = value.filter((item) => {
        const data = item?.data || item?.status || item?.mblog || item;
        return !isNewVideoFlowAd(data);
      });
      for (const item of node[key]) cleanNewVideoFlow(item);
    } else {
      cleanNewVideoFlow(value);
    }
  }
}

function isAdExperimentKey(key) {
  if (typeof key !== "string") return false;
  return /(^p_(?:ad|vad|adx)_|^wb_ad_|(?:^|_)ads?(?:_|$)|Ad(?:Enable|Disable|Support|Fix|Config|Strategy|Display|Show|Card|Video|Banner|Dialog|Splash|Feed|Load|Request))/i.test(key);
}

// 删除广告实验分组，使客户端不再收到这些实验的开关和值。
function removeAdExperiments(node) {
  if (Array.isArray(node)) {
    return node
      .filter((item) => !(typeof item === "string" && isAdExperimentKey(item)))
      .map((item) => removeAdExperiments(item));
  }
  if (!node || typeof node !== "object") return node;
  for (const key of Object.keys(node)) {
    if (isAdExperimentKey(key)) {
      delete node[key];
    } else {
      node[key] = removeAdExperiments(node[key]);
    }
  }
  return node;
}

function removeSearchHotWords(obj) {
  if (!obj || typeof obj !== "object") return;
  if (Array.isArray(obj.data)) obj.data = [];
  if (Array.isArray(obj.rank_list)) obj.rank_list = [];
  if (Object.prototype.hasOwnProperty.call(obj, "personalized")) obj.personalized = 0;
  delete obj.app_link;
  delete obj.h5_link;
  delete obj.quick_app_link;
}
